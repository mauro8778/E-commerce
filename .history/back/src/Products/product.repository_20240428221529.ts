...
export class ProductRepository {
  ...
  async getProductBD() {
    await Promise.all(sheeder.map(async (productData) => {
      ...
      const validarProduct = await this.productrepository.findOne({ where: { name: productName } });

      if (!validarProduct) { // Verifica si el producto NO existe en la base de datos
