...
return product;
  ...
  async createProductRepository(productrepository: Partial<ProductEntity>) {
    ...
    if (productrepository.category && productrepository.category.name) {
        // Verificar si la categoría ya existe
        category = await this.categoryRepository.findOne({ where: { name: productrepository.category.name } });

        if (!category) {
            // Si no existe, crear una nueva categoría
            category = this.categoryRepository.create({ name: productrepository.category.name });
