...
return product;
  ...
  async createProductRepository(productrepository: Partial<ProductEntity>) {
    ...
    let category = await this.categoryRepository.findOne({ where: { name: productrepository.category } });
    
    // Si la categoría no existe, crear una nueva
    if (!category) {
        category = await this.categoryRepository.create({ name: productrepository.category });
