...
return product;
  ...
  async createProductRepository(productrepository: Partial<ProductEntity>,categoryRepository:Partial<CategoryEntity>) {
    ...
    if (!category) {
         await this.categoryRepository.create({ name: productrepository.category.name });
    ...
    await productrepository.category = category;
