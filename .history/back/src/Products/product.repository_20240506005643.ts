...
return product;
  ...
  async createProductRepository(productrepository: Partial<ProductEntity>) {
    const category=await this.categoryRepository.findOne({where:{name:productrepository.category}})
    if(!category){
      const newCategori=this.categoryRepository.create({name:productrepository.category})
