...
return product;
  ...
  async createProductRepository(productrepository: Partial<ProductEntity>) {
    ...
    const categori = await this.categoryRepository.getCategory();
    if(categori.name == productrepository.category){
      
        productrepository.category = categori;
    }
    const newCategori = await this.categoryRepository.create(productrepository.category);
