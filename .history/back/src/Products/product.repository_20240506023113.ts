...
return product;
  ...
  async createProductRepository(productrepository: Partial<ProductEntity>) {
    ...
    if (validarProduct ){if (validarProduct.category) {
      await this.categoryRepository.save(validarProduct.category);
    }}else {await this.categoryRepository.save(productrepository.category);}
