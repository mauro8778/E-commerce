...
export class OrderRepository {
  ...
  async getOrder(id: string, user:UserEntity): Promise<OrderEntity> {
    ...
    const order=await this.orderRepository.findOne({where: { id:users.id } });
  
  if(!order) {
    throw new NotFoundException(`no tiene permiso para ver esta orden`)
  }
  return order;
  }
}
