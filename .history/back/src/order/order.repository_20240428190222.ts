...
export class OrderRepository {
  ...
  async getOrder(userId: string, products: { id: string }[]) {
    ...
    for (let i; i < products.length; i++) {
      ...
      const product =
        await this.productRepository.getProductRepositoryById(productsId);
      if (product && product.stock > 0) {
        product.stock = -1;
        await this.productRepository.updateProductRepository(
          product.id,
          product,
        );
    ...
    return { orderId: savedOrder.id, totalPrice };
  }
