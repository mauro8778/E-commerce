...
export class OrderRepository {
  ...
  async getOrder(id: string,user:Partial<UserEntity>): Promise<OrderEntity> {
    ...
    const order = await this.orderRepository.findOne({where:{id:user.id}})
  
  if(!order){
    throw new NotFoundException(`no tenes acceso a esta direccion `)
  }
  return order ;
}
}
