...
export class OrderRepository {
  ...
  async addOrder(id: string, Products: PartialDto[]) {
    ...
    const productUnico = new Set(productId);
    if (productId.length !== productUnico.size) {
        throw new BadRequestException('La solicitud contiene productos duplicados.');
    }
  ...
  async getOrder(id: string): Promise<OrderEntity> {
    ...
    if (!order) {
      throw new NotFoundException(`no se encontro el pedido con id ${id}`);
  }
